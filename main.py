...
async def reset_bot_state(key, iv, region):
    ...
    try:
        ...
        print("✅ Bot state reset - left squad..! [b][i][FFFF00 YT @NAJMI FF EXPERIMENT .07@NAJMI FF EXPERIMENT .07")
        return True
        ...
        for account_uid, password in accounts_data.items():
            try:
                ...
                if join_packet:
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                    success_count += 1
                    print(f"✅ Join sent from authenticated account: [b][i][FFFF00 YT @NAJMI FF EXPERIMENT .07@NAJMI FF EXPERIMENT .07{account_uid}")
...
async def handle_badge_command(cmd, inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        ...
        for i in range(spam_count):
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
            print(f"✅ Sent /{cmd} request #{i+1} with badge[b][i][FFFF00 YT @NAJMI FF EXPERIMENT .07@NAJMI FF EXPERIMENT .07 {badge_value}")
            await asyncio.sleep(0.1)
        
        success_msg = f"[b][b][b][FFFFFF]✅ Successfully Sent [b][i][FFFF00 YT @NAJMI FF EXPERIMENT .07@NAJMI FF EXPERIMENT .07{spam_count} Join Requests!\n🎯 Target: {target_uid}\n🏷️ Badge: {badge_value}\n"
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if spam_count >= 150:
            completion_msg = f"[b][b][b][FFFFFF]✅ Reject Spam Completed Successfully for ID [b][i][FFFF00 YT @NAJMI FF EXPERIMENT .07@NAJMI FF EXPERIMENT .07{target_uid}\n✅ Total packets sent: {spam_count * 2}\n"
        else:
...
[00FF00]Player Name : [FFFFFF]{player_name}  
[00FF00]Likes Added : [FFFFFF]{likes_added}  
[00FF00]Likes Before : [FFFFFF]{likes_before}  
[00FF00]Likes After : [FFFFFF]{likes_after}  
[C][b][b][11EAFD]‎━━━━━━━━━━━━
[C][b][b][FFB300]Subscribe: [00FF00]SPIDEERIO YT [FFFFFF]!!
"""
...
[00FF00]No Likes Sent!

[FF0000]You have already taken likes with this UID.
Try again after 24 hours.

[00FF00]Player Name : [FF0000]{player_name}  
[00FF00]Likes Before : [FF0000]{likes_before}  
[00FF00]Likes After : [FF0000]{likes_after}  
[C][b][b][FF0000]━━━━━━━━━━━━
"""
        else:
            # ❓ Unexpected case
...
"""
        ...
        return """
[C][b][b][FF0000]━━━━━
[00FF00]Like API Connection Failed!
...
"""
        ...
        return f"""
[C][b][b][FF0000]━━━━━
[00FF00]An unexpected error occurred:
[FF0000]{str(e)}
━━━━━
"""
####################################
...
def get_random_color():
    colors = [
        "[FF0000]", "[FFFFFF]", "[00FF00]", "[FFFF00]", "[FF00FF]", "[00FF00]", "[00FF00]", "[FFA500]",
...
[C][b][b][FFB300]Question: [00FF00]{question}
"""
                        ...
                        if inPuTMsG.strip().startswith('/quick'):
                            ...
                            if len(parts) < 3:
                                error_msg = f"[b][b][FF0000]❌ ERROR! Usage: /quick (team_code) [emote_id] [target_uid]\n\n[00FF00]Examples:\n[FFFFFF]/quick ABC123[00FF00] - Join, send Rings emote, leave\n[FFFFFF]/ghostquick ABC123[00FF00] - Ghost join, send emote, leave\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
...
[b][b][FFC0CB]youtube: NAJMI_FF_EXPERIMENT[/b]

[b][FFC0CB]subcribe: my_channel[00FF00]
 
[b][b][FFA500]telegram: @NAJMI_FF_EXPERIMENT_SOURCES[/b]

[b][FFA500]telegram contact: @NAJMI_FF_EXPERIMENT[A52A2A]
 
Enjoy the bot my friend.......

[b][c][7F00FF] Created by NAJMI_FF_EXPERIMENT 
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                evo_custom_spam_running = False
                                evo_custom_spam_task.cancel()
                                success_msg = f"[b][FFFFFF]✅ SUCCESS! Evolution custom spam stopped successfully! YT @ABHI FF .07\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = """[b][00FF00]✨ ABHI t PRIVATE BOT ✨
                        [00FF00]WELCOME! SEE COMMANDS BELOW 👇"""
    
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
    
                            # Group Commands
                            group_commands = """[b][c][7F00FF]┌── [00FF00]⚡ GROUP COMMANDS ⚡
[b][c][7F00FF]├── [CCFF00]/3  -> [00FF00]3-Player Group
[b][c][7F00FF]├── [CCFF00]/5  -> [00FF00]5-Player Group
[b][c][7F00FF]├── [CCFF00]/6  -> [00FF00]6-Player Group
[b][c][7F00FF]├── [CCFF00]/inv [uid] -> [00FF00]Invite Player
[b][c][7F00FF]├── [CCFF00]! [team_code] -> [00FF00]Join Team
[b][c][7F00FF]├── [CCFF00]/exit -> [00FF00]Leave Group
[b][c][7F00FF]└── [CCFF00]/s -> [00FF00]Start Match"""
    
                            await safe_send_message(response.Data.chat_type, group_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
    
                            # Advanced Commands
                            advanced_commands = """[C][b][FF00FF]┌── [00FF00]⚡ ADVANCED COMMANDS ⚡
[C][b][FF00FF]├── [7F00FF]/spm_inv [uid] -> [00FF00]Spam Invites (30x)
[C][b][FF00FF]├── [7F00FF]/stop spm_inv -> [00FF00]Stop Spam Invites
[C][b][FF00FF]├── [7F00FF]/ghost [code] -> [00FF00]Ghost Join Team
[C][b][FF00FF]├── [7F00FF]/lag [code] -> [00FF00]Lag Attack Team
[C][b][FF00FF]├── [7F00FF]/stop lag -> [00FF00]Stop Lag Attack
[C][b][FF00FF]├── [7F00FF]/reject [uid] -> [00FF00]Reject Spam
[C][b][FF00FF]└── [7F00FF]/invite/[uid] -> [00FF00]Send Join Request"""
    
                            await safe_send_message(response.Data.chat_type, advanced_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
    
                            # Emote Commands
                            emote_commands = """[b][c][7F00FF]┌── [00FF00]⚡ EMOTE COMMANDS ⚡
[b][c][7F00FF]├── [00FF00]/e [uid] [id] -> [00FF00]Send Single Emote
[b][c][7F00FF]├── [00FF00]/fast [uid] [id] -> [00FF00]Fast Emote (25x)
[b][c][7F00FF]└── [00FF00]/p [uid] [id] [x] -> [00FF00]Custom Emote (X times)"""
    
                            await safe_send_message(response.Data.chat_type, emote_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
    
                            # Evolution Emote Commands
                            evo_commands = """[C][b][FF00FF]┌── [00FF00]⚡ EVOLUTION EMOTES ⚡
[C][b][FF00FF]├── [00FF00]/evo [uid] [1-21] -> [00FF00]Send Evolution Emote
[C][b][FF00FF]├── [00FF00]/evo_fast [uid] [1-21] -> [00FF00]Fast Evo (25x)
[C][b][FF00FF]├── [00FF00]/evo_c [uid] [1-21] [x] -> [00FF00]Custom Evo (X times)
[C][b][FF00FF]├── [00FF00]@evos [uid] -> [00FF00]Auto Cycle All Evo Emotes
[C][b][FF00FF]└── [00FF00]@sevos -> [00FF00]Stop Evo Emote Cycle"""
    
                            await safe_send_message(response.Data.chat_type, evo_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
    
                            # AI & Utility Commands
                            ai_commands = """[b][c][7F00FF]┌── [00FF00]⚡ AI & UTILITY COMMANDS ⚡
[b][c][7F00FF]├── [FFFFFF]/ai [question] -> [00FF00]Ask AI Anything
[b][c][7F00FF]├── [FFFFFF]/likes [uid] -> [00FF00]Send 100 Likes
[b][c][7F00FF]├── [FFFFFF]/admin -> [00FF00]Admin Information
[b][c][7F00FF]└── [FFFFFF]/status -> [00FF00]Bot Status Check"""
                           ...
                           # Badges command 
                            badge_commands = """[C][b][FF00FF]┌── [00FF00]⚡BADGE JOIN REQUESTS⚡
[C][b][FF00FF] |     
[C][b][FF00FF]├── [FF0000]/s1 [uid] 
-> [00FF00]Join Req Craftland Badge
[C][b][FF00FF]├── [FF0000]/s2 [uid] 
-> [00FF00]Join Req New V-Badge
[C][b][FF00FF]├── [FF0000]/s3 [uid] 
-> [00FF00]Join Req Moderator Badge
[C][b][FF00FF]├── [FF0000]/s4 [uid] 
-> [00FF00]Join Req Small V-Badge
[C][b][FF00FF]├── [FF0000]/s5 [uid] 
-> [00FF00]Join Req Pro Badge"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # Footer
                            footer = """[C][b][1E90FF]╭━━━━━━━━━╮
[1E90FF]┃ [00FF00]CREDITS: @ABHI_FF [1E90FF]
[1E90FF]┃ [00FF00]YOUTUBE @ABHI FF .07[1E90FF]
[1E90FF]┃ [00FF00]DEVELOPER: ABHI FF [1E90FF]
[1E90FF]┃ [00FF00]BOT STATUS: ONLINE[1E90FF]
[1E90FF]┃ [00FF00]ABHI[1E90FF]
[1E90FF]╰━━━━━━━━━╯"""
...
async def MaiiiinE():
    Uid , Pw = '4377194683','640A2D45E6F3CEA825F555BC732708D4822A4E3220BEA7810EA233EE377F54E6'
